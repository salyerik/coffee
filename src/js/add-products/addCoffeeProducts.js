...
import { ROOT_COFFEE_LIST, ROOT_MAINCOFFEE_LIST } from '../constants/root.js'
...
class ProductsCoffee {
	...
	selectWeight(event) {
		...
		COFFEEPRODUCTS.forEach(({ id, price, oldPrice }) => {
			if (idProduct === id) {
				if (weightIndex === 0) {
	...
	sortLowHigh(COFFEEPRODUCTS) {
		COFFEEPRODUCTS.sort((a, b) => a.price[0] > b.price[0] ? 1 : -1);
	}

	sortHighLow(COFFEEPRODUCTS) {
		COFFEEPRODUCTS.sort((a, b) => a.price[0] < b.price[0] ? 1 : -1);
	}

	sortByRating(COFFEEPRODUCTS) {
		COFFEEPRODUCTS.sort((a, b) => a.rating < b.rating ? 1 : -1);
	}

	sortByReview(COFFEEPRODUCTS) {
		COFFEEPRODUCTS.sort((a, b) => a.reviews < b.reviews ? 1 : -1);
	}
	...
	render(methodSort) {
		...
		if (methodSort == 'sortHighLow') {
			this.sortHighLow(COFFEEPRODUCTS);
		} else if (methodSort == 'sortLowHigh') {
			this.sortLowHigh(COFFEEPRODUCTS);
		} else if (methodSort == 'rating') {
			this.sortByRating(COFFEEPRODUCTS);
		} else if (methodSort == 'review') {
			this.sortByReview(COFFEEPRODUCTS);
		}

		COFFEEPRODUCTS.forEach(({ id, name, descr, img, weight, price, oldPrice, reviews, rating }) => {
			let activeClass = '';
			let activeText = '';
		...
		if (ROOT_MAINCOFFEE_LIST) {
			ROOT_MAINCOFFEE_LIST.innerHTML = htmlCatalog;
		}
		if (ROOT_COFFEE_LIST) {
