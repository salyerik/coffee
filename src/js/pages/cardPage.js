...
import { COFFEEPRODUCTS } from '../constants/listCoffee.js'
...
export default function () {
	...
	const cardTotalNoDiscount = document.querySelector('.card-bottom__pre-price span');
	const cardTotalDiscount = document.querySelector('.card-bottom__discount span');
	let summCatalog = 0;
	...
	function totalCardElements() {
		if (products === 1) {
			...
			cardQuantity.innerHTML = 'Basket is empty';
		}
	}
	...
	class Card {
		handleSetLocationStorage(element, id) {
			...
			let productPrice = element.parentElement.children[2].children[1].innerHTML;
	...
	cardCoffeePage.render(COFFEEPRODUCTS);

	const cardTeaPage = new Card();
	cardTeaPage.render(TEAPRODUCTS);

	const cardFoodPage = new Card();
	cardFoodPage.render(FOODPRODUCTS);

	const cardVendingPage = new Card();
	cardVendingPage.render(VENDINGPRODUCTS, 'cardVendingPage', 'VENDINGPRODUCTS');
	...
	minusBtns.forEach(btn => (
		btn.addEventListener('click', () => {
			cardCoffeePage.quantityMinus(event, COFFEEPRODUCTS);
			cardTeaPage.quantityMinus(event, TEAPRODUCTS);
			cardFoodPage.quantityMinus(event, FOODPRODUCTS);
			cardVendingPage.quantityMinus(event, VENDINGPRODUCTS);
		})
	))
	const plusBtns = document.querySelectorAll('.card-top__btn_plus')
	plusBtns.forEach(btn => (
		btn.addEventListener('click', () => {
			cardCoffeePage.quantityPlus(event, COFFEEPRODUCTS);
			cardTeaPage.quantityPlus(event, TEAPRODUCTS);
			cardFoodPage.quantityPlus(event, FOODPRODUCTS);
			cardVendingPage.quantityPlus(event, VENDINGPRODUCTS);
		})
	))
